...
async function _setRoute53_(subdomain_, domain_, ip_) {
  ...
  console.log(`[${moment().format()}] At ${moment().format('llll')}; IP ${ip} (input IP: ${ip_})`);
  ...
  console.log(`[${moment().format()}] Attempting to set IP ${ip} (input IP: ${ip_}) of ${subdomain}.${domain} in Route53.`);
  try {
    ...
    if (HostedZones.length > 0) {
      ...
      if (zone.Name === `${domain}.`) {
        ...
        console.log(`[${moment().format()}] At ${moment().format('llll')}: requesting change of "${subdomain}.${domain}" to  ${ip}`);

        const result = await route53.waitFor('resourceRecordSetsChanged', {Id: ChangeInfo.Id}).promise();

        console.log({result});
        console.log(`[${moment().format()}] At ${moment().format('llll')}: changed "${subdomain}.${domain}" to  ${ip}`);
